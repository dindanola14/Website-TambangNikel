<?php
	...
	if(isset($_POST['proses'])){
        ...
        $data = mysqli_query($conn,"SELECT* FROM penyetuju WHERE user='$user'");
        $cek = mysqli_num_rows($data);
        if($cek > 0){
            $_SESSION['user'] = $user;
            $_SESSION['status'] = "login";
            echo '<script>alert("Login Sukses");window.location="indexpenyetuju.php"</script>';
        }else{
            echo '<script>alert("Maaf! Data yang anda masukan salah.");history.go(-1);</script>';
        }
	}
    if(isset($_SESSION['status']))
    {header('location: indexpenyetuju.php');
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<link rel="icon" href="logo-tambangnikel.png">
  ...
  <title>Login Penyetuju | Tambang Nikel</title>
  <meta charset="utf-8">
