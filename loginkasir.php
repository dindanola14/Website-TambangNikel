<?php
	...
	if(isset($_POST['proses'])){
        ...
        $data = mysqli_query($conn,"SELECT* FROM kasir WHERE user='$user'");
        $cek = mysqli_num_rows($data);
        if($cek > 0){
            $_SESSION['user'] = $user;
            $_SESSION['status'] = "login";
            echo '<script>alert("Login Sukses");window.location="indexkasir.php"</script>';
        }else{
...
<link rel="icon" href="cafe.png">
  ...
  <title>Login Kasir | Cafe Telkom</title>
  <meta charset="utf-8">
